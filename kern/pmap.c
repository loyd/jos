...
boot_alloc(uint32_t n)
{
	...
	// nextfree.  Nextfree is kept aligned to a multiple of PGSIZE.
	
	void *result = nextfree;

	nextfree += ROUNDUP(n, PGSIZE);

	if (n >= BOOTMEM || nextfree >= (char *)KADDR(BOOTMEM)) {
...
mem_init(void)
{
	...
	size_t pages_len = npages * sizeof(struct PageInfo);
	pages = boot_alloc(pages_len);
	memset(pages, 0, pages_len);

	//////////////////////////////////////////////////////////////////////
	// Make 'envs' point to an array of size 'NENV' of 'struct Env'.

	size_t envs_len = NENV * sizeof(struct Env);
	envs = boot_alloc(envs_len);
	memset(envs, 0, envs_len);

	//////////////////////////////////////////////////////////////////////
	// Make 'vsys' point to an array of size 'NVSYSCALLS' of int.
	size_t vsys_len = sizeof(int32_t) * NVSYSCALLS;
	vsys = boot_alloc(vsys_len);
	memset(vsys, 0, vsys_len);
	...
	boot_map_region(kern_pgdir, UENVS, PTSIZE - PGSIZE, PADDR(envs), PTE_U);
...
page_init(void)
{
	...
	assert(PADDR(boot_alloc(0)) >= EXTPHYSMEM);
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_link != NULL || pp->pp_ref != 0) {
		panic("page_free: invalid page %x", page2pa(pp));
	}
